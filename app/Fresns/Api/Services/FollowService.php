...
class FollowService
{
    ...
    public function getCommentListByFollowAll(int $authUserId, ?string $contentType = null, ?string $dateLimit = null)
    {
        ...
        // follow user post
        $userCommentQuery = Comment::with(['creator', 'post', 'hashtags'])->whereIn('user_id', $followUserIds)->where('is_anonymous', 0)->isEnable()->latest();
        // follow group post
        $groupCommentQuery = Comment::with(['creator', 'post', 'hashtags'])->where('digest_state', 2)->isEnable()->latest();
        $groupCommentQuery->whereHas('post', function ($query) use ($followGroupIds) {
            $query->whereIn('group_id', $followGroupIds);
        });
        // follow hashtag post
        $hashtagCommentQuery = Comment::with(['creator', 'post', 'hashtags'])->where('digest_state', 2)->isEnable()->latest();
        $hashtagCommentQuery->whereHas('hashtags', function ($query) use ($followHashtagIds) {
            $query->whereIn('hashtags.id', $followHashtagIds);
        });
        // digest post query
        $digestCommentQuery = Post::with(['creator', 'post', 'hashtags'])->where('digest_state', 3)->latest();
    ...
    public function getCommentListByFollowUsers(int $authUserId, ?string $contentType = null, ?string $dateLimit = null)
    {
        ...
        $commentQuery = Comment::with(['creator', 'post', 'hashtags'])->whereIn('user_id', $followUserIds)->where('is_anonymous', 0)->isEnable()->latest();
    ...
    public function getCommentListByFollowGroups(int $authUserId, ?string $contentType = null, ?string $dateLimit = null)
    {
        ...
        $commentQuery = Comment::with(['creator', 'post', 'hashtags'])->where('top_parent_id', 0)->isEnable()->latest();
