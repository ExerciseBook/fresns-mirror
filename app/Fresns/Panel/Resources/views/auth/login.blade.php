...
@section('content')
  <main class="form-signin text-center">
    <form method="post" class="p-3" action="{{route('panel.login')}}">
      ...
      <h4 class="mb-3 fw-normal">{{ @trans('panel::panel.language') }}</h4>
      ...
      <h4 class="mb-3 fw-normal">{{@trans('panel::panel.login')}}</h4>
      <div class="form-floating">
        <input type="text" class="form-control rounded-bottom-0" name="username" value="{{ old('email') }}" placeholder="name@example.com">
        <label for="account">{{ @trans('panel::panel.account') }}</label>
      </div>
      <div class="form-floating">
        <input type="password" class="form-control rounded-top-0 border-top-0" name="password" placeholder="Password">
        <label for="password">{{ @trans('panel::panel.password') }}</label>
      </div>
      <button type="submit" class="w-100 btn btn-lg btn-primary mt-4">{{ @trans('panel::panel.enter') }}</button>
